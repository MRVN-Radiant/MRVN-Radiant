...
public:
    ...
    explicit BasicVector2(const Element &value) :
        m_elements{value, value} {}

          Element &x()       { return m_elements[0]; }
    const Element &x() const { return m_elements[0]; }

          Element &y()       { return m_elements[1]; }
    const Element &y() const { return m_elements[1]; }

          Element &operator[](std::size_t i)       { return m_elements[i]; }
    const Element &operator[](std::size_t i) const { return m_elements[i]; }

          Element *data()       { return m_elements; }
    const Element *data() const { return m_elements; }
...
public:
    ...
    explicit BasicVector3(const Element &value) :
        m_elements{value, value, value} {}
    ...
    const Element &y() const { return m_elements[1]; }
          ...
          Element &z()       { return m_elements[2]; }
    const Element &z() const { return m_elements[2]; }
...
public:
    ...
    const Element &z() const { return m_elements[2]; }
          ...
          Element &w()       { return m_elements[3]; }
    const Element &w() const { return m_elements[3]; }

    Element  index(std::size_t i) const { return m_elements[i]; }
    Element &index(std::size_t i)       { return m_elements[i]; }

    Element  operator[](std::size_t i) const { return m_elements[i]; }
    Element &operator[](std::size_t i)       { return m_elements[i]; }
...
template<typename Element>
inline       Element *vector3_to_array(      BasicVector3<Element> &self) { return self.data(); }

template<typename Element>
inline const Element *vector3_to_array(const BasicVector3<Element> &self) { return self.data(); }

template<typename Element>
inline       Element *vector4_to_array(      BasicVector4<Element> &self) { return self.data(); }

template<typename Element>
inline const Element *vector4_to_array(const BasicVector4<Element> &self) { return self.data(); }
